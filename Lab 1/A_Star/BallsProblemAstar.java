...
public class BallsProblemAstar
{
	...
	public BallsProblemAstar(String input){
	...
	public void runAstar()
	{
		...
		while(true)
		{
			...
			if(isRightPlace(popState)){
				...
				System.out.print("Final Result(A*): ");
	...
	{
		int barrier = (state.dash>=size/2) ? (size/2-1) : size/2;
		int dist = 0;
		for(int i=0; i<=barrier; i++)
		{
			...
			{
				int j=i;
				while(barrier+j<size && state.s[barrier+j]!='M')
				{
					j++;
				}
				dist += barrier+(j-i);
	...
	public static void main(String[] args)
	{
		...
		BallsProblemAstar bp = new BallsProblemAstar(line);
		bp.runAstar();
	}				
}